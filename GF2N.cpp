...
unsigned long long GF2Nmult(unsigned int a, unsigned int b) {
    __m128i prod;
    ...
    __m128i qplus = _mm_set_epi32(0,0,0,0b1001100); // x^16 mod(generator polynomial) = x^6 + x^3 + x^2. This needs to be precalculated
    __m128i pol = _mm_set_epi32(0,0,0,0b11101); // The t least significant terms of the polynomial
    ...
    product = _mm_cvtsi128_si64(prod>>N); // Select the first s most significant terms of a 2s-1 polynomial (it has 2s elements, remove the last s elements). The result is a s-1 degree polynomial
