...
void max_interesting_states() {
    ...
    if (max_G_distances_file.is_open() && max_R_distances_file.is_open()) {
        ...
        state = domain_wall_state(n_qubits, qubitstate_size);
...
void get_state_distances(const unsigned int &n_qubits, const unsigned int &qubitstate_size, Eigen::VectorXcd &state, double &distance_G, double &distance_R) {
    Eigen::MatrixXd Qfunc = pure_Qfunc_from_operational(n_qubits, qubitstate_size, state);
    ...
    get_Gfunc(n_qubits, qubitstate_size, sym_Qfunc, Gfunc);
...
void get_state_distances(const unsigned int &n_qubits, const unsigned int &qubitstate_size, Eigen::VectorXd &state, double &distance_G, double &distance_R) {
    Eigen::MatrixXd Qfunc = pure_Qfunc_from_operational(n_qubits, qubitstate_size, state);
    Eigen::Tensor<double, 3> sym_Qfunc = get_symQ(n_qubits, qubitstate_size, Qfunc);
    Eigen::Tensor<double, 3> Rmnk = get_Rmnk(n_qubits);
    Eigen::Tensor<double, 3> Gfunc;
